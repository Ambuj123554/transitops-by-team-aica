  ...
  return (
    <aside className="fixed left-0 top-0 h-screen w-64 bg-slate-900 flex flex-col z-40 border-r border-slate-800">
      <div className="px-5 py-5 border-b border-slate-800">
        <span className="text-white font-bold tracking-tight text-xl">TransitOps</span>
      </div>
      ...
      <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
        {NAV_ITEMS.map(item => {
          ...
          if (!accessible) {
            return (
              <div key={item.href} className="flex items-center gap-3.5 px-4 py-2.5 rounded-md opacity-30 cursor-not-allowed select-none">
          ...
          return (
            <Link
              ...
              href={item.href}
              className={cn(
                'flex items-center gap-3.5 px-4 py-2.5 rounded-md text-sm transition-colors cursor-pointer',
                isActive
      ...
      <div className="px-3 py-4 border-t border-slate-800">
        <button
          onClick={logout}
          className="flex items-center gap-3.5 px-4 py-2.5 w-full rounded-md text-sm text-slate-400 hover:text-white hover:bg-slate-800 transition-colors cursor-pointer"
        >
