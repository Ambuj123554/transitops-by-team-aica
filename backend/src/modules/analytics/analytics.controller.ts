...
const ANALYTICS_ROLES = ['FLEET_MANAGER', 'FINANCIAL_ANALYST'];

function requireAnalyticsRole(role?: string): boolean {
  return role ? ANALYTICS_ROLES.includes(role) : false;
}
...
export async function getMonthlyRevenue(req: Request, res: Response, next: NextFunction) {
  try {
    if (!requireAnalyticsRole(req.user?.role)) {
...
export async function exportCsv(req: Request, res: Response, next: NextFunction) {
  try {
    if (!requireAnalyticsRole(req.user?.role)) {
      return sendSuccess(res, 'Only Fleet Manager and Financial Analyst can export reports');
    }
